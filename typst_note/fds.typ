...
== 1. 核心概念：后进先出 (LIFO)
#pad(left: 1em)[
  **栈**是一种特殊的受限线性数据结构。它的核心规则是**后进先出** (Last-In-First-Out, LIFO)。

  - 你只能从最上面放一个新的进去（入栈）。
  - 你也只能从最上面拿走一个旧的（出栈）。
  - 最先放进去的被压在最底下，必须等上面的全拿走，最后才能拿出来。
]


== 2. 核心操作接口 (Operations)
#pad(left: 1em)[
  在代码实现中，我们通常会为栈 S 定义以下几个核心“动作”：
  
  - `InitStack(S)`: **初始化**。在内存中开辟空间，凭空造出一个空栈。
  - `IsEmpty(S)`: **判空**。检查栈里面是不是空的。在执行出栈前必须检查，防止“拿空气”导致程序崩溃。
  - `Push(S, X)`: **压栈 / 入栈**。把新元素 X 强行压入栈的顶部。
  - `Pop(S)`: **弹栈 / 出栈**。把栈顶的元素弹出来，并在栈中删除它。
  - `Top(S)` 或 `Peek(S)`: **查看栈顶**。只偷偷看一眼最上面的元素是什么，但不把它拿走（不改变栈的结构）。
]
...
== 3. 深入理解：我们为什么需要栈？
#pad(left: 1em)[
  栈在计算机科学中无处不在，它简直就是为了处理**“历史回溯”**和**“层层嵌套”**而生的：
  
  - **程序内存的“栈区”**：就像我们刚才讨论的，函数 A 调用函数 B，B 调用 C。执行完 C 必须精确回到 B，B 执行完回到 A。这种“最后调用的最先结束”的逻辑，底层就是靠栈这种数据结构来管理的。
  - **浏览器的“后退”按钮**：你刚刚浏览的网页历史被依次“压”入栈中，点击后退，就是一次“弹栈”操作，让你回到上一个页面。
  - **代码括号匹配**：编译器在检查你的代码里 `{ [ ( ) ] }` 这些括号是否成对合法时，核心算法也是借助栈来完成的。
]

以下是实现的核心结构体定义：
```c
struct StackRecord {
    int Capacity ;    /* 栈的大小（容量） */
    int TopOfStack;   /* 栈顶指针（数组索引） */
    /* 工作原理：Push 时 ++，Pop 时 --，-1 表示栈空 */
    ElementType *Array; /* 存储栈元素的数组指针 */
};
```
...
== 关键变量解析：
#pad(left: 1em)[
- **`TopOfStack` (栈顶指针):** 这是数组实现的核心核心核心。它不是内存指针，而是一个数组索引，指向当前栈顶元素的位置。
    - **初始化：** 当栈为空时，`TopOfStack` 设为 **-1**（因为有效的数组索引从 0 开始）。
    - **Push (入栈):** 先将 `TopOfStack` 加 1，然后将新元素放入 `Array[TopOfStack]`。
    - **Pop (出栈):** 将 `TopOfStack` 减 1（有效地移除了顶部的元素）。
]


// 3. 补充说明 (Note)
#text(weight: "bold")[重要 Note:]
#grid(
  columns: (1fr, 1fr),
  gutter: 1em,
  rect(
    ...
    inset: 1em,
    [
      #text(weight: "bold")[① 封装 (Encapsulation)]
     
      栈的模型必须是高度**封装**的。除了栈的例程外，代码的任何其他部分都**不**允许直接尝试访问 `Array` 或 `TopOfStack` 变量。这有助于隐藏内部实现细节，防止外部意外破坏。
    ]
  ),
  rect(
    fill: rgb("#d3d3d3"),
    ...
    inset: 1em,
    [
      #text(weight: "bold")[② 错误检查 (Error check)]
     
      在执行 **Push** 之前必须检查栈是否已满（防止数组越界和栈溢出）。在执行 **Pop** (或 **Top**) 之前，必须检查栈是否为空（防止读取空内存和栈下溢）。
    ]
  )
)


= 3. 栈的应用：符号平衡问题 (Balancing Symbols)

== 1. 核心概念与目标
在编写代码或解析表达式时，我们必须确保所有的圆括号 `()`、方括号 `[]` 和花括号 `{}` 都能**正确嵌套**并**平衡配对**。
- 合法例子：`{ [ ( ) ] }`
- 非法例子：`( ) )` (右括号多余)、`( [ )` (括号类型不匹配)、`( [ ]` (左括号未闭合)

**为什么用栈？**
因为嵌套本质上是“后进先出 (LIFO)”的逻辑。当你遇到一个右括号时，它必须去匹配**最近一次**遇到的那个左括号。栈“只关注栈顶”的特性完美契合了这个需求。

== 2. 核心算法逻辑

```c
Algorithm {
    Make an empty stack S; // 1. 初始化一个空栈，准备记录
    
    while (read in a character c) { // 2. 从左到右逐个读取字符
        if (c is an opening symbol)  /* ( [ { */
            Push(c, S); // 遇到开符号：无条件压入栈中，作为“待匹配”的任务
            
        else if (c is a closing symbol) /* ) ] } */ {
            // 遇到闭符号，开始检查栈的状态：
            if (S is empty) { ERROR; exit; } // 错误1：栈是空的，说明没有左括号等它，右括号多余了！
            else { 
                if (Top(S) doesn't match c) { ERROR, exit; } // 错误2：如栈顶是 '('，但读到了 ']'，类型不匹配！
                else Pop(S); // 完美匹配：将栈顶对应的开符号弹出，完成一次配对
            }
        }
    }
    
    // 3. 结算阶段
    if (S is not empty) ERROR; // 读完所有字符后，如果栈里还有东西，说明有左括号“孤老终生”，报错！
}
```


= 4. 栈的应用：后缀表达式求值 (Postfix Evaluation)

== 1. 为什么需要后缀表达式？
人类习惯使用**中缀表达式** (如 `a + b * c`)，但这需要处理复杂的优先级和括号。
计算机更喜欢**后缀表达式**（又称逆波兰表达式，Reverse Polish notation，如 `a b c * +`）。它**没有括号，不看优先级**，从左到右扫描一遍就能直接出结果。

== 2. 基于栈的求值规则
非常简单粗暴的“两步走”逻辑：
- **情况 A**：遇到**数字 (Operand)**，直接压入栈 (Push)。
- **情况 B**：遇到**符号 (Operator)**，连续弹出 (Pop) 栈顶的两个数字进行计算（先弹出的在运算符右侧，后弹出的在左侧），然后将**计算结果重新压入栈中**。

== 3. 实战推演：`6 2 / 3 - 4 2 * +`
我们来看看计算机是如何用栈一步一步算出结果 `8` 的：

1. 读 `6` -> 压栈。当前栈：`[6]`
2. 读 `2` -> 压栈。当前栈：`[6, 2]` 
3. 读 `/` -> 弹 `2` 和 `6`，算 `6 / 2 = 3`，压栈。当前栈：`[3]`
4. 读 `3` -> 压栈。当前栈：`[3, 3]`
5. 读 `-` -> 弹 `3` 和 `3`，算 `3 - 3 = 0`，压栈。当前栈：`[0]`
6. 读 `4` -> 压栈。当前栈：`[0, 4]` *(注：这是幻灯片配图中展示的瞬间)*
7. 读 `2` -> 压栈。当前栈：`[0, 4, 2]`
8. 读 `*` -> 弹 `2` 和 `4`，算 `4 * 2 = 8`，压栈。当前栈：`[0, 8]`
9. 读 `+` -> 弹 `8` 和 `0`，算 `0 + 8 = 8`，压栈。当前栈：`[8]`
10. **结束**。栈顶剩下的 `8` 即为最终结果。


= 5. 栈的应用：中缀转后缀 (Infix to Postfix)

== 1. 转换的黄金法则

在将人类习惯的表达式（如 `a + b * c - d`）转换为机器喜欢的后缀表达式时，我们遵循以下规则：

- **操作数 (数字/字母)**：它们的相对顺序在转换前后**绝对不变**。读到它们，直接追加到输出结果的末尾，不与栈发生任何关系。
- **运算符 (+ - * /)*：使用一个栈来暂存它们，并利用栈的特性来处理优先级。

== 2. 运算符进栈的“硬性规定”

当读到一个新的运算符时，不能无脑压栈，必须和当前的**栈顶运算符**进行比较：

1. 如果栈是空的，直接入栈。
2. 比较优先级：
   - 如果栈顶符号的优先级 **大于或等于** 新符号，说明栈顶的运算必须先执行。此时将栈顶符号**弹出并输出**，然后再次比较新的栈顶，直到栈顶优先级低于新符号为止，最后将新符号压栈。
   - 如果栈顶符号的优先级 **低于** 新符号（例如图中栈顶是 `+`，新来的是 `*`），说明新符号优先级更高，直接将新符号**压栈**。

== 3. 处理括号 `( )` 的特殊逻辑
- 遇到左括号 `(`：直接无条件压栈，它就像一个隔离墙。
- 遇到右括号 `)`：不断弹出栈顶符号并输出，直到遇到最近的左括号 `(` 为止。然后把这一对括号直接丢弃（后缀表达式不需要括号）。

== 4. 结尾收尾
当整个中缀表达式全部读取完毕后，如果栈里还有剩余的运算符，就把它们依次全部弹出并追加到输出结果的末尾。
