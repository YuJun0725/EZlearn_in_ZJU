...
除了微弱的表面张力外，流体内部是不存在拉力的。

对于平衡流体，是不存在表面摩擦力的，只有表面静压力。
...
证明思路是，取立方体微元，使用质量力和表面力平衡方程求解。

不同面的静压强大小可以通过泰勒展开由一点压强表示，

这个方程式适用于平衡流体

方程式说明：如果存在质量力，则表面静压强一定会在相应方向上发生变化。如果可以忽略质量力，则静压强处处相等。

质量力的势函数

$ d p = -d W $
等压面也就是等势面，与单位 质量力矢量垂直。

对于两个不相混的平衡液体的交界面必为等压面。
